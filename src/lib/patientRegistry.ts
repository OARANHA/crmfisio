...
const patientPayload = (input: PatientRegistryInput) => ({
...
export async function createPatientRegistry(input: PatientRegistryInput): Promise<string> {
  const { data, error } = await supabase.rpc('create_patient_registry_v2', {
    p_patient: patientPayload(input),
...
export async function updatePatientRegistry(patientId: string, input: PatientRegistryInput): Promise<void> {
  const { error } = await supabase.rpc('update_patient_registry_v2', {
...
export async function uploadPatientAvatar(userId: string, patientId: string, file: File): Promise<string> {
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) throw new Error('Formato de imagem não suportado. Use JPG, PNG ou WEBP.');
  ...
  const path = `${clinicId}/${patientId}/avatar.${extension}`;
  const { error: uploadError } = await supabase.storage.from('patient-avatars').upload(path, file, { upsert: true, contentType: file.type, cacheControl: '3600' });
  if (uploadError) throw uploadError;

  const { error: pathError } = await supabase.rpc('set_patient_avatar_path', { p_patient_id: patientId, p_avatar_path: path });
  if (pathError) throw pathError;
  return path;
}

export async function removePatientAvatar(patientId: string, currentPath?: string | null): Promise<void> {
  if (currentPath) {
    const { error: storageError } = await supabase.storage.from('patient-avatars').remove([currentPath]);
    if (storageError) throw storageError;
  }
  const { error } = await supabase.rpc('set_patient_avatar_path', { p_patient_id: patientId, p_avatar_path: null });
  if (error) throw error;
}

export async function getPatientAvatarUrl(patientId: string): Promise<string | null> {
  const { data, error } = await supabase.from('patients').select('avatar_path').eq('id', patientId).maybeSingle();
  if (error) throw error;
  if (!data?.avatar_path) return null;
  const { data: signed, error: signedError } = await supabase.storage.from('patient-avatars').createSignedUrl(data.avatar_path, 30 * 60);
  if (signedError) throw signedError;
  return signed?.signedUrl ?? null;
}

export async function listPatientGuardians(patientId: string): Promise<PatientGuardian[]> {
  const { data, error } = await supabase.from('patient_guardians').select('*').eq('patient_id', patientId).order('is_primary_contact', { ascending: false }).order('created_at', { ascending: true });
  if (error) throw error;
  return (data ?? []).map((row) => ({
    id: row.id,
    ...
    isEmergencyContact: row.is_emergency_contact ?? false,
