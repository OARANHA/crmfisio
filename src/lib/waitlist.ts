...
export type UpdateWaitlistEntryInput = Omit<CreateWaitlistEntryInput, 'clinicId' | 'patientId'>;

const mapEntry = (row: Record<string, unknown>): WaitlistEntry => ({
  id: String(row.id), clinicId: String(row.clinic_id), patientId: String(row.patient_id),
  ...
  period: (row.period ?? 'qualquer') as WaitlistPeriod, priority: Number(row.priority ?? 0),
  notes: row.notes ? String(row.notes) : '', status: (row.status ?? 'aguardando') as WaitlistStatus,
...
export async function loadWaitlist(clinicId: string): Promise<WaitlistEntry[]> {
  const { data, error } = await supabase.from('waitlist_entries').select('*').eq('clinic_id', clinicId)
    .in('status', ['aguardando', 'ofertado']).order('priority', { ascending: false }).order('created_at', { ascending: true });
...
export async function createWaitlistEntry(input: CreateWaitlistEntryInput): Promise<WaitlistEntry> {
  const { data, error } = await supabase.from('waitlist_entries').insert({
    clinic_id: input.clinicId, patient_id: input.patientId, professional_id: input.professionalId ?? null,
    unit_id: input.unitId ?? null, preferred_days: input.preferredDays ?? [], period: input.period ?? 'qualquer',
    priority: input.priority ?? 0, notes: input.notes?.trim() || null,
...
export async function updateWaitlistEntry(entryId: string, input: UpdateWaitlistEntryInput): Promise<void> {
  const { error } = await supabase.from('waitlist_entries').update({
    professional_id: input.professionalId ?? null, unit_id: input.unitId ?? null,
    preferred_days: input.preferredDays ?? [], period: input.period ?? 'qualquer',
    priority: input.priority ?? 0, notes: input.notes?.trim() || null,
  }).eq('id', entryId);
  if (error) throw error;
}
...
export async function claimWaitlistSlot(entryId: string, cancelledAppointmentId: string): Promise<void> {
  const { error } = await (supabase.rpc as Function)('claim_waitlist_slot', {
    p_waitlist_id: entryId, p_cancelled_appointment_id: cancelledAppointmentId,
  });
  if (error) throw error;
}

export const WAITLIST_PERIOD_LABEL: Record<WaitlistPeriod, string> = {
  manha: 'Manhã', tarde: 'Tarde', noite: 'Noite', qualquer: 'Qualquer horário',
};
