...
/**
 * Estado canônico do domínio clínico longitudinal.
 *
 * Evoluções, consentimentos e NPS deixam de pertencer ao store global. Isso
 * mantém mutações e falhas do domínio isoladas e prepara paginação/carga sob
 * demanda sem forçar um reload completo da clínica.
 */
export function ClinicalProvider({ children }: { children: ReactNode }) {
  ...
  const refreshClinical = useCallback(async () => {
    ...
    try {
      ...
      console.error('[MedicsPro] domínio clínico:', cause);
      setError('Não foi possível carregar os dados clínicos.');
      throw cause;
    } finally {
      setLoading(false);
    }
  ...
  const addEvolution = useCallback(async (evolution: Omit<Evolution, 'id'>) => {
    if (!clinicId) throw new Error('Clínica não identificada');
    const created = await insertEvolution(clinicId, evolution);
    setEvolutions((current) => [created, ...current]);
    return created;
  }, [clinicId]);

  const signConsent = useCallback(async (id: string) => {
  ...
  const answerNps = useCallback(async (id: string, nota: number) => {
    ...
    try {
      await updateSurvey(id, nota);
    } catch (cause) {
      if (previous) {
        const rollback = previous;
        setSurveys((current) => current.map((survey) => survey.id === id ? rollback : survey));
      }
      throw cause;
    }
  }, []);

  const value = useMemo<ClinicalState>(() => ({
    evolutions,
    consents,
    surveys,
    loading,
    ...
    signConsent,
    answerNps,
  }), [evolutions, consents, surveys, loading, error, refreshClinical, addEvolution, signConsent, answerNps]);
...
export function useClinical(): ClinicalState {
  ...
  if (!context) throw new Error('useClinical deve ser usado dentro de ClinicalProvider');
  return context;
}
