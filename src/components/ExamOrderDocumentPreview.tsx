...
import type { ExamOrderPayload } from '../lib/clinicalExamOrder';
import { useCurrentUserAccess } from '../lib/currentUserAccess';
import { buildExamOrderDocumentHtml } from '../lib/examOrderPrintRenderer';
import type { Patient } from '../lib/types';

type ExamOrderDocumentPreviewProps = {
  patient: Patient;
...
export function ExamOrderDocumentPreview({ patient, payload, renderDefinition, clinic }: ExamOrderDocumentPreviewProps) {
  ...
  const html = buildExamOrderDocumentHtml({
    ...
    mode: 'draft',
    context: {
      patient: { name: patient.preferredName || patient.nome, birthDate: patient.nascimento },
      clinic: { name: clinic.name || 'Clínica', address: clinic.address, phone: clinic.phone },
      issuer: {
        name: user?.nome || 'Profissional solicitante',
  ...
  return (
    <section className="self-start overflow-hidden rounded-2xl border border-line/65 bg-panel 2xl:sticky 2xl:top-4" data-exam-order-live-preview="draft">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line/65 px-4 py-3">
        ...
        <span className="rounded-full border border-amber/35 bg-amber/[0.08] px-2.5 py-1 text-[9.5px] font-semibold uppercase tracking-[0.08em] text-amber">Rascunho · não emitido</span>
      </div>
      <div className="bg-deep/35 p-3 sm:p-4">
        <iframe
          title="Pré-visualização do pedido de exames"
          srcDoc={html}
          sandbox=""
