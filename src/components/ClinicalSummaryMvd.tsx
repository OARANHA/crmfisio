  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-line bg-panel">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-line px-5 py-4">
        <div>
          <h2 className="font-display text-[16px] font-semibold">Visão clínica</h2>
          <p className="mt-0.5 text-[12.5px] text-fog">O essencial para entender o paciente antes de atender.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-[12px]">
          ...
          {readiness.map((item) => (
            <span
              key={item.label}
              className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 font-medium ${item.ok ? 'bg-mint/[0.08] text-mint' : 'bg-amber/[0.08] text-amber'}`}
            >
      ...
      <div className="grid lg:grid-cols-[1.15fr_.85fr]">
        ...
        <div className="px-5 py-5">
          <p className="text-[11.5px] font-semibold uppercase tracking-[0.07em] text-fog">Linha do cuidado</p>
          ...
          {activeSessionTime && (
            <div className="mt-4 rounded-xl bg-amber/[0.07] px-3.5 py-3 text-[12.5px] leading-relaxed text-amber">
...
function ClinicalDatum({ label, value, className = '', emphasis = false }: { label: string; value: string; className?: string; emphasis?: boolean }) {
  return (
    <div className={className}>
      <p className="text-[11.5px] font-semibold uppercase tracking-[0.06em] text-fog">{label}</p>
      <p className={`mt-1.5 leading-relaxed ${emphasis ? 'text-[15px] font-medium text-paper' : 'text-[13.5px] text-paper/90'}`}>{value}</p>
    </div>
  );
}
...
function SessionDatum({ label, value, alert = false }: { label: string; value: string; alert?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-4 py-3 first:pt-0 last:pb-0">
      ...
      <span className={`text-right text-[13px] font-medium ${alert ? 'text-amber' : 'text-paper/90'}`}>{value}</span>
    </div>
  );
}
