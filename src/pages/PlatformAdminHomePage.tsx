      ...
      <div className="mx-auto flex min-h-screen max-w-[1580px]">
        <aside className="hidden w-[250px] shrink-0 border-r border-line/60 bg-panel/75 p-5 lg:flex lg:flex-col">
          ...
          <nav className="space-y-1.5 text-[12px] font-semibold">
            <PlatformNav to="/platform" icon="⌂" label="Visão geral" active />
            <PlatformNav icon="↗" label="Comercial" badge="em preparação" />
            <PlatformNav to="/platform/modulos" icon="◇" label="Clientes & Plataforma" />
            <PlatformNav icon="◌" label="Receita & Assinaturas" badge="em preparação" />
          </nav>
          ...
          <nav className="space-y-1 text-[11.5px] font-medium">
            <PlatformNav to="/platform/provisionar" icon="＋" label="Onboarding" />
            <PlatformNav to="/platform/governanca" icon="⌁" label="Governança" />
          </nav>

          <div className="mt-auto rounded-[20px] border border-line/70 bg-deep/60 p-4">
            <div className="flex items-center gap-2">
              <span className={`h-2.5 w-2.5 rounded-full ${automationHealthy ? 'bg-mint' : 'bg-amber'}`} />
              <p className="text-[11.5px] font-semibold">{automationHealthy ? 'Operação saudável' : 'Requer atenção'}</p>
            </div>
            <p className="mt-2 text-[10.5px] leading-relaxed text-fog">{pausedAutomations} automação(ões) pausada(s) · {failedRuns} ciclo(s) com falha recente.</p>
          </div>
        </aside>
        ...
        <div className="min-w-0 flex-1">
          <header className="sticky top-0 z-20 border-b border-line/55 bg-ink/90 px-5 py-4 backdrop-blur-xl md:px-7">
            <div className="flex flex-wrap items-center gap-3">
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-mint">MedicsPro Platform Admin</p>
                <h1 className="mt-0.5 font-display text-[20px] font-bold tracking-tight">Visão geral</h1>
              </div>
              <button type="button" onClick={() => void refresh()} disabled={loadingData} className="rounded-xl border border-line bg-panel px-3.5 py-2.5 text-[11px] font-semibold text-fog transition hover:border-mint/35 hover:text-paper disabled:opacity-50">{loadingData ? 'Atualizando…' : 'Atualizar dados'}</button>
          ...
          <main className="space-y-5 p-4 md:p-6 xl:p-7">
            {error && <div className="rounded-xl border border-amber/35 bg-amber/[0.05] px-4 py-3 text-[12px] text-amber">{error}</div>}

            <div className="flex gap-2 overflow-x-auto pb-1 lg:hidden">
              <MobileArea label="Visão geral" active />
              <MobileArea label="Comercial" />
              <MobileArea label="Clientes & Plataforma" />
              <MobileArea label="Receita & Assinaturas" />
            </div>
            ...
            <section className="grid gap-4 xl:grid-cols-[1.65fr_0.75fr]">
              <div className="relative overflow-hidden rounded-[26px] border border-mint/20 bg-gradient-to-br from-mint/[0.12] via-panel to-panel p-6 md:p-7">
                ...
                <div className="relative max-w-3xl">
                  ...
                  <p className="mt-3 max-w-2xl text-[12.5px] leading-relaxed text-fog">Acompanhe clínicas, onboarding e saúde operacional agora. Comercial e receita entram aqui quando as fontes reais estiverem conectadas.</p>
              ...
              <div className="rounded-[26px] border border-line bg-panel p-5.5 shadow-[0_14px_36px_rgba(3,16,48,0.045)]">
                ...
                <div className="mt-5 h-2 overflow-hidden rounded-full bg-deep">
                  <div className={`h-full rounded-full ${automationHealthy ? 'w-full bg-mint' : failedRuns > 0 ? 'w-1/2 bg-amber' : 'w-3/4 bg-amber'}`} />
                </div>
            ...
            <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <MetricCard label="Clínicas ativas" value={String(activeClinics)} detail={`${data.clinics.length} cadastrada(s) no total`} tone="mint" icon="◇" />
              <MetricCard label="Solicitações pendentes" value={String(data.pendingRequests.length)} detail="aguardando análise" tone={data.pendingRequests.length ? 'amber' : 'aqua'} icon="＋" />
              <MetricCard label="Clínicas suspensas" value={String(suspendedClinics)} detail="lifecycle sob controle" tone={suspendedClinics ? 'amber' : 'fog'} icon="‖" />
              <MetricCard label="Base ativa" value={`${activePercent}%`} detail="proporção de clínicas ativas" tone={activePercent === 100 ? 'mint' : 'aqua'} icon="↗" />
            </section>

            <section className="grid gap-4 xl:grid-cols-[1.35fr_0.65fr]">
              <div className="rounded-[24px] border border-line bg-panel p-5 md:p-6 shadow-[0_14px_36px_rgba(3,16,48,0.035)]">
                <div className="flex flex-wrap items-start gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-fog">Operação da base</p>
                    <h3 className="mt-1 font-display text-[19px] font-bold">Distribuição atual da plataforma</h3>
                    <p className="mt-1 text-[11.5px] text-fog">Leitura factual da base cadastrada e da fila de onboarding.</p>
                  </div>
                  <Link to="/platform/modulos" className="text-[11px] font-semibold text-mint hover:text-paper">Ver clientes →</Link>
                </div>

                <div className="mt-6 space-y-5">
                  <DistributionBar label="Clínicas ativas" value={activeClinics} max={Math.max(data.clinics.length, 1)} tone="mint" />
                  <DistributionBar label="Clínicas suspensas" value={suspendedClinics} max={Math.max(data.clinics.length, 1)} tone="amber" />
                  <DistributionBar label="Solicitações pendentes" value={data.pendingRequests.length} max={Math.max(data.clinics.length + data.pendingRequests.length, 1)} tone="aqua" />
                </div>

                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  <BusinessAreaCard title="Comercial" eyebrow="Área 2" description="Leads, oportunidades e conversões entrarão aqui quando o bridge n8n/CRM estiver conectado." status="Fonte de dados em preparação" />
                  <BusinessAreaCard title="Receita & Assinaturas" eyebrow="Área 4" description="MRR, pagamentos e inadimplência serão exibidos somente após integração factual de billing." status="Asaas planejado para fase posterior" />
                </div>
              </div>

              <div className="rounded-[24px] border border-line bg-panel p-5 md:p-6 shadow-[0_14px_36px_rgba(3,16,48,0.035)]">
                <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-fog">Clientes</p>
                <h3 className="mt-1 font-display text-[19px] font-bold">Base ativa</h3>
                <div className="mx-auto mt-5 grid h-44 w-44 place-items-center rounded-full" style={{ background: `conic-gradient(var(--color-mint) 0 ${activePercent}%, var(--color-line) ${activePercent}% 100%)` }}>
                  <div className="grid h-32 w-32 place-items-center rounded-full bg-panel text-center shadow-inner">
                    <div>
                      <p className="text-[10px] text-fog">Ativas</p>
                      <p className="font-display text-[30px] font-bold">{activeClinics}</p>
                      <p className="text-[10px] text-fog">de {data.clinics.length}</p>
                    </div>
                  </div>
                </div>
                <div className="mt-5 space-y-2.5 text-[11px]">
                  <LegendRow label="Ativas" value={activeClinics} tone="mint" />
                  <LegendRow label="Suspensas" value={suspendedClinics} tone="line" />
                  <LegendRow label="Em análise" value={data.pendingRequests.length} tone="amber" />
                </div>
              </div>
            </section>
            ...
            <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-[24px] border border-line bg-panel p-5 md:p-6">
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-fog">Onboarding</p>
                    <h3 className="mt-1 font-display text-[19px] font-bold">Solicitações de acesso</h3>
                  </div>
                  <Link to="/platform/provisionar" className="text-[11px] font-semibold text-mint hover:text-paper">Abrir fila →</Link>
                </div>
                <div className="mt-4 space-y-2">
                  {data.pendingRequests.slice(0, 5).map((request) => (
                    <Link key={request.id} to="/platform/provisionar" className="flex flex-wrap items-center gap-3 rounded-2xl border border-line/70 bg-deep/45 px-4 py-3.5 transition hover:border-mint/30 hover:bg-raise/30">
                      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-mint/[0.09] font-display font-bold text-mint">{request.clinicName.slice(0, 1).toUpperCase()}</div>
                      <div className="min-w-0 flex-1">
                      ...
                      <div className="text-right"><span className="rounded-full border border-amber/30 bg-amber/[0.07] px-2.5 py-1 text-[9.5px] font-semibold text-amber">pendente</span><p className="mt-1 text-[9px] text-fog">{formatDate(request.createdAt)}</p></div>
                    </Link>
                  ))}
                  {!loadingData && data.pendingRequests.length === 0 && <div className="rounded-2xl border border-dashed border-line p-6 text-center text-[11.5px] text-fog">Nenhuma solicitação pendente neste momento.</div>}
                </div>
              </div>

              <div className="rounded-[24px] border border-line bg-panel p-5 md:p-6">
                <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-fog">Governança</p><h3 className="mt-1 font-display text-[19px] font-bold">Atividade recente</h3></div><Link to="/platform/governanca" className="text-[11px] font-semibold text-mint hover:text-paper">Auditoria →</Link></div>
                <div className="mt-4 space-y-2">
                  {data.audit.slice(0, 6).map((entry) => <div key={entry.id} className="rounded-2xl border border-line/65 bg-deep/40 px-3.5 py-3"><div className="flex items-start gap-3"><span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-mint" /><div className="min-w-0 flex-1"><p className="truncate text-[11.5px] font-semibold text-paper">{auditLabel(entry.action)}</p><p className="mt-0.5 truncate text-[9.5px] text-fog">{entry.entityType} · {entry.entityKey}</p></div><span className="shrink-0 text-[9px] text-fog">{formatDate(entry.createdAt)}</span></div></div>)}
            ...
            <section className="rounded-[24px] border border-line bg-panel p-5 md:p-6">
              <div className="flex flex-wrap items-center gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-fog">Clientes & Plataforma</p>
                  <h3 className="mt-1 font-display text-[19px] font-bold">Clínicas recentes</h3>
                </div>
                <div className="flex gap-2"><Link to="/platform/modulos" className="rounded-xl border border-line bg-deep/45 px-3.5 py-2.5 text-[11px] font-semibold text-paper">Módulos e planos</Link><Link to="/platform/provisionar" className="rounded-xl bg-mint px-3.5 py-2.5 text-[11px] font-semibold text-on-accent">Novo onboarding</Link></div>
              </div>
...
function PlatformNav({ to, icon, label, active = false, badge }: { to?: string; icon: string; label: string; active?: boolean; badge?: string }) {
  const content = <><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-xl ${active ? 'bg-mint text-on-accent' : 'bg-deep text-fog'}`}>{icon}</span><span className="min-w-0 flex-1 truncate">{label}</span>{badge && <span className="rounded-full border border-line px-2 py-1 text-[8.5px] font-medium text-fog">{badge}</span>}</>;
  const className = `flex items-center gap-2.5 rounded-2xl px-2.5 py-2 transition ${active ? 'bg-mint/[0.08] text-paper' : 'text-fog hover:bg-deep hover:text-paper'}`;
  return to ? <Link to={to} className={className}>{content}</Link> : <div className={`${className} cursor-default opacity-80`}>{content}</div>;
}

function MobileArea({ label, active = false }: { label: string; active?: boolean }) {
  return <span className={`shrink-0 rounded-full border px-3 py-2 text-[10.5px] font-semibold ${active ? 'border-mint/30 bg-mint/[0.08] text-mint' : 'border-line bg-panel text-fog'}`}>{label}</span>;
}

function MetricCard({ label, value, detail, tone, icon }: { label: string; value: string; detail: string; tone: 'mint' | 'aqua' | 'amber' | 'fog'; icon: string }) {
  const toneClass = tone === 'mint' ? 'text-mint' : tone === 'aqua' ? 'text-aqua' : tone === 'amber' ? 'text-amber' : 'text-fog';
  return <article className="rounded-[22px] border border-line bg-panel p-4.5 shadow-[0_10px_28px_rgba(3,16,48,0.03)]"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="text-[10px] font-semibold text-fog">{label}</p><p className={`mt-2 font-display text-[29px] font-bold tracking-tight ${toneClass}`}>{value}</p><p className="mt-1 text-[10.5px] leading-relaxed text-fog">{detail}</p></div><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full border border-line bg-deep ${toneClass}`}>{icon}</span></div></article>;
}
...
function DistributionBar({ label, value, max, tone }: { label: string; value: number; max: number; tone: 'mint' | 'amber' | 'aqua' }) {
  const width = Math.max(value > 0 ? 4 : 0, Math.min(100, Math.round((value / max) * 100)));
  const fill = tone === 'mint' ? 'bg-mint' : tone === 'amber' ? 'bg-amber' : 'bg-aqua';
  return <div><div className="mb-2 flex items-center justify-between gap-3 text-[11px]"><span className="font-semibold text-paper/85">{label}</span><span className="font-display font-bold text-paper">{value}</span></div><div className="h-3 overflow-hidden rounded-full bg-deep"><div className={`h-full rounded-full ${fill} transition-[width] duration-500`} style={{ width: `${width}%` }} /></div></div>;
}

function BusinessAreaCard({ eyebrow, title, description, status }: { eyebrow: string; title: string; description: string; status: string }) {
  return <article className="rounded-2xl border border-dashed border-line bg-deep/35 p-4"><p className="text-[9.5px] font-semibold uppercase tracking-[0.12em] text-fog">{eyebrow}</p><p className="mt-1.5 font-display text-[14px] font-bold">{title}</p><p className="mt-2 text-[10.5px] leading-relaxed text-fog">{description}</p><p className="mt-3 text-[9.5px] font-semibold text-mint">{status}</p></article>;
}

function LegendRow({ label, value, tone }: { label: string; value: number; tone: 'mint' | 'amber' | 'line' }) {
  const dot = tone === 'mint' ? 'bg-mint' : tone === 'amber' ? 'bg-amber' : 'bg-line2';
