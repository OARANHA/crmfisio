-- MedicsPro — PC-PTSD-5 Clinician-Assisted Administration V1
...
BEGIN
  IF to_regclass('public.nexus_result_contracts') IS NULL
    ...
    RAISE EXCEPTION 'pcptsd5_clinician_assisted_prerequisite_missing';
  END IF;
END;
$$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE module_key='scales' AND tool_key='pcptsd5'
      AND rule_key='nexus.pcptsd5' AND rule_version='nexus-pcptsd5-ptbr-ops-2026-09-16'
      AND required_capability IS DISTINCT FROM 'nexus.scales'
  ) THEN
    RAISE EXCEPTION 'pcptsd5_nexus_contract_conflict';
  END IF;
  IF EXISTS (
    ...
    WHERE instrument_key='pcptsd5'
      AND (
        ...
        OR engine_tool_key IS DISTINCT FROM 'pcptsd5'
        OR engine_rule_key IS DISTINCT FROM 'nexus.pcptsd5'
        OR engine_rule_version IS DISTINCT FROM 'nexus-pcptsd5-ptbr-ops-2026-09-16'
      )
  ) THEN
    RAISE EXCEPTION 'pcptsd5_neutral_catalog_conflict';
  END IF;
END;
$$;
...
INSERT INTO public.clinical_instrument_catalog(
  ...
  'pcptsd5', 'nexus', 'scales', 'pcptsd5',
  'nexus.pcptsd5', 'nexus-pcptsd5-ptbr-ops-2026-09-16', true
)
ON CONFLICT (instrument_key) DO NOTHING;
-- PC-PTSD-5 V1 uses an operational PT-BR translation; no Brazilian validation
-- is claimed. It is deliberately clinician-assisted only and fail-closed.
