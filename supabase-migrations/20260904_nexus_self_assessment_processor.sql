...
-- Processor foundation for submitted self-assessments.
-- Clinical calculation stays server-side; browser never creates finalized results.
...
CREATE INDEX IF NOT EXISTS idx_nexus_self_assessment_processing
  ON public.nexus_self_assessment_invites(scale_key, rule_version, submitted_at)
  WHERE status = 'submitted' AND processed_result_id IS NULL;
...
BEGIN
  ...
  WITH picked AS (
    ...
    WHERE i.status = 'submitted'
      ...
      AND (
        i.processing_started_at IS NULL
...
REVOKE ALL ON FUNCTION public.release_nexus_self_assessment_claim(uuid,text) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  IF v_invite.status <> 'submitted'
     OR v_invite.submitted_at IS NULL
     OR v_invite.processing_started_at IS NULL THEN
  ...
  -- The existing Nexus triggers require auth.uid() to be the clinical author.
  -- This RPC is service-role only and temporarily installs the invite author
  -- as transaction-local JWT context, preserving all existing trigger checks.
  v_previous_claims := current_setting('request.jwt.claims', true);
  PERFORM set_config(
    'request.jwt.claims',
    jsonb_build_object('sub', v_invite.professional_id::text, 'role', 'authenticated')::text,
    true
  );
  ...
  INSERT INTO public.nexus_clinical_results (
    ...
    v_invite.appointment_id,
    p_result->>'moduleKey',
    p_result->>'toolKey',
    p_result->>'ruleKey',
    p_result->>'ruleVersion',
    p_result->>'requiredCapability',
    'draft',
    p_result->'inputSnapshot',
    p_result->'outputSnapshot',
    nullif(p_result->>'totalScore', '')::numeric,
    nullif(p_result->>'maxScore', '')::numeric,
    nullif(p_result->>'classification', ''),
    nullif(p_result->>'severity', ''),
    nullif(p_result->>'interpretation', ''),
    nullif(p_result->>'soapText', ''),
    p_result->'evidenceSnapshot'
  )
  RETURNING id INTO v_result_id;

  FOR v_flag IN SELECT value FROM jsonb_array_elements(p_red_flags)
  LOOP
    IF jsonb_typeof(v_flag) IS DISTINCT FROM 'object' THEN
      RAISE EXCEPTION 'Red flag inválida';
    END IF;
  ...
  UPDATE public.nexus_self_assessment_invites
     SET processed_result_id = v_result_id,
         status = 'processed',
