-- MedicsPro — D2-E2 Referral Professional Print Renderer V1
...
VALUES (
  ...
  '{"layout":"clinical-document/referral-v1","title":"Encaminhamento clínico","show_clinic_address":true,"show_clinic_phone":true,"show_patient_birth_date":true,"show_specialty":true}'::jsonb,
...
BEGIN
  ...
  WHERE template_id = '12000000-0000-4000-8000-000000000007'::uuid
    AND version = 2;

  IF v.id IS DISTINCT FROM '12100000-0000-4000-8000-000000000016'::uuid
     OR v.published_at IS NULL
     OR v.definition IS DISTINCT FROM '{"kind":"referral","fields":["recipient","reason","clinical_summary","requested_action","priority","observations"]}'::jsonb
     ...
     OR v.render_definition->>'layout' IS DISTINCT FROM 'clinical-document/referral-v1'
     OR v.render_definition->>'title' IS DISTINCT FROM 'Encaminhamento clínico'
    ...
    RAISE EXCEPTION 'clinical_referral_renderer_v1_version_conflict';
  END IF;
END $$;

-- Promote only from the canonical D2-E0 v1. On replay the pointer is already v2,
-- so this UPDATE is intentionally a no-op and does not churn updated_at.
...
WHERE t.id = '12000000-0000-4000-8000-000000000007'::uuid
  ...
  AND t.current_version_id = '12100000-0000-4000-8000-000000000007'::uuid;
