...
-- Additive backend foundation only. No Prescription UI, PDF/signature service,
-- Nexus medication catalog integration, or document types beyond the two below.
...
ALTER TABLE public.clinical_document_templates
  ...
  REFERENCES public.clinical_document_template_versions(id)
  ON DELETE RESTRICT;
...
CREATE TABLE IF NOT EXISTS public.clinical_documents (
  ...
  CONSTRAINT clinical_documents_lifecycle_shape CHECK (
    (
      ...
      AND payload_snapshot IS NULL AND context_snapshot IS NULL
      AND template_definition_snapshot IS NULL AND rendered_snapshot IS NULL
    ...
    OR (
      ...
      AND canceled_at IS NULL AND canceled_by IS NULL AND cancel_reason IS NULL
      AND payload_snapshot IS NOT NULL AND context_snapshot IS NOT NULL
      AND template_definition_snapshot IS NOT NULL
      AND btrim(coalesce(rendered_snapshot, '')) <> ''
      AND btrim(coalesce(renderer_version, '')) <> ''
    )
    OR (
      status = 'canceled'
      AND issued_at IS NOT NULL AND issued_by IS NOT NULL
...
CREATE INDEX IF NOT EXISTS clinical_documents_patient_history_idx
  ON public.clinical_documents (clinic_id, patient_id, issued_at DESC)
  WHERE status IN ('issued', 'canceled');
...
BEGIN
  ...
  IF NEW.clinic_id IS DISTINCT FROM OLD.clinic_id
     ...
     OR NEW.encounter_record_id IS DISTINCT FROM OLD.encounter_record_id
     OR NEW.issuer_id IS DISTINCT FROM OLD.issuer_id
     OR NEW.document_type IS DISTINCT FROM OLD.document_type
     OR NEW.template_id IS DISTINCT FROM OLD.template_id
     OR NEW.template_version_id IS DISTINCT FROM OLD.template_version_id
     OR NEW.document_identifier IS DISTINCT FROM OLD.document_identifier
  ...
  IF OLD.status IN ('issued', 'canceled') AND NEW IS DISTINCT FROM OLD THEN
    IF OLD.status = 'issued'
       ...
       AND NEW.issued_by IS NOT DISTINCT FROM OLD.issued_by THEN
      RETURN NEW;
    END IF;
...
CREATE TRIGGER trg_clinical_document_templates_updated_at
BEFORE UPDATE ON public.clinical_document_templates
FOR EACH ROW EXECUTE FUNCTION public.clinical_document_set_updated_at();
...
CREATE TRIGGER trg_clinical_document_template_version_immutable
BEFORE UPDATE OR DELETE ON public.clinical_document_template_versions
FOR EACH ROW EXECUTE FUNCTION public.guard_clinical_document_template_version_immutable();

DROP TRIGGER IF EXISTS trg_clinical_document_integrity ON public.clinical_documents;
CREATE TRIGGER trg_clinical_document_integrity
BEFORE UPDATE OR DELETE ON public.clinical_documents
FOR EACH ROW EXECUTE FUNCTION public.guard_clinical_document_integrity();

DROP TRIGGER IF EXISTS trg_clinical_document_event_append_only ON public.clinical_document_events;
CREATE TRIGGER trg_clinical_document_event_append_only
BEFORE UPDATE OR DELETE ON public.clinical_document_events
FOR EACH ROW EXECUTE FUNCTION public.guard_clinical_document_event_append_only();
...
CREATE OR REPLACE FUNCTION public.assert_clinical_document_actor(
...
BEGIN
  IF auth.uid() IS NULL
     OR public.current_user_can_issue_clinical_document(p_document_type) IS NOT TRUE THEN
    RAISE EXCEPTION 'clinical_document_eligibility_required' USING ERRCODE = '42501';
  END IF;
  ...
  IF a.id IS NULL
     OR a.clinic_id IS DISTINCT FROM public.current_clinic_id()
     OR a.fisio_id IS DISTINCT FROM auth.uid()
     OR a.status IS DISTINCT FROM 'em_atendimento' THEN
    RAISE EXCEPTION 'clinical_document_own_active_encounter_required' USING ERRCODE = '42501';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'clinical_document_patient_link_invalid' USING ERRCODE = '23514';
  END IF;
...
CREATE OR REPLACE FUNCTION public.assert_clinical_document_cancel_actor(p_document_id uuid)
...
BEGIN
  ...
  IF d.id IS NULL OR d.status <> 'issued' THEN
    RAISE EXCEPTION 'clinical_document_issued_required' USING ERRCODE = '42501';
  END IF;

  IF auth.uid() IS NULL
     OR d.clinic_id IS DISTINCT FROM public.current_clinic_id()
     OR d.issuer_id IS DISTINCT FROM auth.uid()
     OR public.current_user_can_issue_clinical_document(d.document_type) IS NOT TRUE THEN
    RAISE EXCEPTION 'clinical_document_cancel_actor_required' USING ERRCODE = '42501';
  END IF;
...
AS $$
BEGIN
  IF p_document_type NOT IN ('medication_prescription','therapeutic_guidance')
...
BEGIN
  ...
  WHERE id = p_template_version_id AND published_at IS NOT NULL;

  SELECT * INTO t
  FROM public.clinical_document_templates
  WHERE id = v.template_id AND status = 'active';

  IF t.id IS NULL
     OR t.current_version_id IS DISTINCT FROM v.id
     OR (t.owner_type = 'clinic' AND t.clinic_id IS DISTINCT FROM public.current_clinic_id()) THEN
    RAISE EXCEPTION 'clinical_document_template_unavailable' USING ERRCODE = '42501';
  END IF;
  ...
  INSERT INTO public.clinical_documents(
    ...
    document_type, template_id, template_version_id, issuer_id, payload,
    document_identifier
  )
  VALUES(
    did,
    ...
    a.paciente_id,
    a.id,
    (SELECT id FROM public.clinical_encounter_records WHERE appointment_id = a.id),
    t.document_type,
    t.id,
    v.id,
    auth.uid(),
    p_payload,
    'DOC-' || to_char(current_date, 'YYYYMMDD') || '-' || upper(replace(did::text, '-', ''))
  )
  RETURNING * INTO d;

  INSERT INTO public.clinical_document_events(document_id, clinic_id, event_type, actor_id)
  VALUES(d.id, d.clinic_id, 'created', auth.uid());

  RETURN d;
END;
$$;

CREATE OR REPLACE FUNCTION public.save_clinical_document_draft(
  p_document_id uuid,
  p_payload jsonb
)
RETURNS public.clinical_documents
...
BEGIN
  ...
  IF a.paciente_id IS DISTINCT FROM d.patient_id
     OR a.clinic_id IS DISTINCT FROM d.clinic_id THEN
    RAISE EXCEPTION 'clinical_document_context_invalid' USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  snap := jsonb_build_object(
    'patient', jsonb_build_object(
      'id', d.patient_id,
      'name', (SELECT nome FROM public.patients WHERE id = d.patient_id)
    ),
...
CREATE OR REPLACE FUNCTION public.cancel_clinical_document(
  p_document_id uuid,
  p_reason text
)
RETURNS public.clinical_documents
...
BEGIN
  IF nullif(btrim(coalesce(p_reason, '')), '') IS NULL THEN
    RAISE EXCEPTION 'clinical_document_cancel_reason_required' USING ERRCODE = '22023';
  END IF;

  d := public.assert_clinical_document_cancel_actor(p_document_id);
  ...
  INSERT INTO public.clinical_document_events(document_id, clinic_id, event_type, actor_id, metadata)
  VALUES(
    d.id,
    d.clinic_id,
    'canceled',
    auth.uid(),
    jsonb_build_object('reason', d.cancel_reason)
  );
...
USING (
  ...
  AND status = 'active'
  AND public.current_user_can_issue_clinical_document(document_type)
  AND (
    owner_type = 'platform'
...
USING (
  ...
  AND EXISTS (
    ...
    WHERE t.id = template_id
      AND t.status = 'active'
      AND public.current_user_can_issue_clinical_document(t.document_type)
      AND (
        t.owner_type = 'platform'
        OR (t.owner_type = 'clinic' AND t.clinic_id = public.current_clinic_id())
      )
  )
);

DROP POLICY IF EXISTS clinical_documents_read_clinical ON public.clinical_documents;
CREATE POLICY clinical_documents_read_clinical
...
CREATE POLICY clinical_document_events_read_clinical
ON public.clinical_document_events
FOR SELECT TO authenticated
USING (
  clinic_id = public.current_clinic_id()
  AND EXISTS (
    ...
    WHERE d.id = document_id
      AND public.can_access_patient_clinical_record(d.patient_id)
  )
);

REVOKE ALL ON public.clinical_document_templates,
  public.clinical_document_template_versions,
  public.clinical_documents,
  public.clinical_document_events
FROM PUBLIC, anon, authenticated;

GRANT SELECT ON public.clinical_document_templates,
  public.clinical_document_template_versions,
  public.clinical_documents,
  public.clinical_document_events
TO authenticated;

GRANT ALL ON public.clinical_document_templates,
  public.clinical_document_template_versions,
  public.clinical_documents,
  public.clinical_document_events
TO service_role;
...
REVOKE ALL ON FUNCTION public.assert_clinical_document_actor(uuid,text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.assert_clinical_document_payload_ready(text,jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.assert_clinical_document_cancel_actor(uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.render_clinical_document_snapshot(text,text,jsonb,jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.create_clinical_document_draft(uuid,uuid,jsonb) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.save_clinical_document_draft(uuid,jsonb) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.issue_clinical_document(uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.cancel_clinical_document(uuid,text) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.current_user_can_issue_clinical_document(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.create_clinical_document_draft(uuid,uuid,jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.save_clinical_document_draft(uuid,jsonb) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.issue_clinical_document(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.cancel_clinical_document(uuid,text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.assert_clinical_document_actor(uuid,text) TO service_role;
GRANT EXECUTE ON FUNCTION public.assert_clinical_document_payload_ready(text,jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.assert_clinical_document_cancel_actor(uuid) TO service_role;
...
INSERT INTO public.clinical_document_template_versions(
  id, template_id, version, definition, render_definition, variables_contract, published_at
)
VALUES
  ...
  ('12100000-0000-4000-8000-000000000003','12000000-0000-4000-8000-000000000003',1,'{"kind":"therapeutic_guidance","fields":["items","patient_instructions","observations"]}','{"layout":"clinical-document/plain-text-v1"}','["patient.name","issuer.name","issuer.registro","appointment.id"]',now()),
  ('12100000-0000-4000-8000-000000000004','12000000-0000-4000-8000-000000000004',1,'{"kind":"therapeutic_guidance","fields":["items","patient_instructions","observations"]}','{"layout":"clinical-document/plain-text-v1"}','["patient.name","issuer.name","issuer.registro","appointment.id"]',now())
ON CONFLICT (id) DO NOTHING;
