...
\echo '3) fisio direct insert/update fails closed outside own agenda'
SELECT (
...
\echo '4) structural/source fields require canonical flow'
SELECT (
  pg_get_functiondef('public.guard_appointment_mutation_boundary()'::regprocedure)
...
\echo '5) canonical reschedule RPC remains installed'
...
\echo '6) clinical self-transition and status workflow guards remain installed'
SELECT
  EXISTS (
