...
: "${PGPORT:=5432}"
: "${PGUSER:=postgres}"
: "${PGDATABASE:=clinical_exam_order_renderer_v1_test}"
export PGHOST PGPORT PGUSER PGDATABASE

if [[ "$PGDATABASE" != "clinical_exam_order_renderer_v1_test" ]]; then
  echo "Refusing to run D2-D2 harness outside clinical_exam_order_renderer_v1_test" >&2
  exit 1
fi
...
# Replay only D2-D2 and prove that published versions, current pointer and issued rows remain byte-for-byte stable.
"${PSQL[@]}" <<'SQL'
CREATE TEMP TABLE d2d2_before AS
SELECT
  (SELECT jsonb_agg(to_jsonb(t) ORDER BY t.id) FROM public.clinical_document_templates t WHERE t.id='12000000-0000-4000-8000-000000000006'::uuid) AS template_row,
  (SELECT jsonb_agg(to_jsonb(v) ORDER BY v.version) FROM public.clinical_document_template_versions v WHERE v.template_id='12000000-0000-4000-8000-000000000006'::uuid) AS version_rows,
  (SELECT count(*) FROM public.clinical_documents WHERE document_type='exam_order') AS exam_documents,
  (SELECT count(*) FROM public.clinical_document_events e JOIN public.clinical_documents d ON d.id=e.document_id WHERE d.document_type='exam_order') AS exam_events;

\i supabase-migrations/20260912_clinical_exam_order_renderer_v1.sql
\i supabase-verifiers/VERIFY_20260912_CLINICAL_EXAM_ORDER_RENDERER_V1.sql

DO $$
DECLARE b record;
BEGIN
  SELECT * INTO b FROM d2d2_before;
  IF b.template_row IS DISTINCT FROM (SELECT jsonb_agg(to_jsonb(t) ORDER BY t.id) FROM public.clinical_document_templates t WHERE t.id='12000000-0000-4000-8000-000000000006'::uuid)
     OR b.version_rows IS DISTINCT FROM (SELECT jsonb_agg(to_jsonb(v) ORDER BY v.version) FROM public.clinical_document_template_versions v WHERE v.template_id='12000000-0000-4000-8000-000000000006'::uuid)
     OR b.exam_documents <> (SELECT count(*) FROM public.clinical_documents WHERE document_type='exam_order')
     OR b.exam_events <> (SELECT count(*) FROM public.clinical_document_events e JOIN public.clinical_documents d ON d.id=e.document_id WHERE d.document_type='exam_order') THEN
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_replay_mutated_state';
  END IF;
END $$;
SQL

echo 'CLINICAL EXAM ORDER RENDERER V1 POSTGRESQL 16 PASS'
