-- MedicsPro — PCPTSD5 Clinician-Assisted V1 installed-contract verifier.
BEGIN;
SET TRANSACTION READ ONLY;
SET LOCAL statement_timeout = '30s';

\echo 'VERIFY PCPTSD5 clinician-assisted V1'

DO $$
BEGIN
  IF current_setting('transaction_read_only') <> 'on' THEN
    RAISE EXCEPTION 'pcptsd5_v1_verifier_not_read_only';
  END IF;
  IF current_setting('server_version_num')::integer < 160000 THEN
    RAISE EXCEPTION 'pcptsd5_v1_postgresql_16_required';
  END IF;
  IF to_regclass('public.nexus_result_contracts') IS NULL
    ...
    RAISE EXCEPTION 'pcptsd5_v1_prerequisite_missing';
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE module_key='scales' AND tool_key='pcptsd5'
      AND rule_key='nexus.pcptsd5'
      AND rule_version='nexus-pcptsd5-ptbr-ops-2026-09-16'
      AND required_capability='nexus.scales'
  ) THEN RAISE EXCEPTION 'pcptsd5_v1_nexus_contract_missing'; END IF;

  IF public.resolve_nexus_result_required_capability(
       'scales','pcptsd5','nexus.pcptsd5','nexus-pcptsd5-ptbr-ops-2026-09-16'
     ) IS DISTINCT FROM 'nexus.scales' THEN
    RAISE EXCEPTION 'pcptsd5_v1_resolver_drift';
  END IF;
...
BEGIN
  ...
  IF v_default IS NULL OR lower(v_default) NOT IN ('false', 'false::boolean') THEN
    RAISE EXCEPTION 'pcptsd5_v1_setting_default_not_deny';
  END IF;
  ...
  IF position('clinic_clinical_instrument_settings' in v_src)=0
    ...
    RAISE EXCEPTION 'pcptsd5_v1_neutral_authorization_drift';
  END IF;
  ...
  IF position('can_apply_clinical_instrument_in_encounter' in v_src)=0
    ...
    RAISE EXCEPTION 'pcptsd5_v1_writer_authorization_drift';
  END IF;
END;
$$;
...
BEGIN
  IF to_regclass('public.clinical_instrument_patient_self_contracts') IS NOT NULL THEN
    EXECUTE 'SELECT EXISTS (SELECT 1 FROM public.clinical_instrument_patient_self_contracts WHERE instrument_key=$1)'
      INTO v_has_patient_self USING 'pcptsd5';
  END IF;
  IF v_has_patient_self THEN
    RAISE EXCEPTION 'pcptsd5_v1_patient_self_surface_widened';
  END IF;
END;
$$;

\echo 'PC-PTSD-5 CLINICIAN ASSISTED V1 VERIFY PASSED'
ROLLBACK;
