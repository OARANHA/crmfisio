-- MedicsPro — D2-E2 production-safe verifier
...
BEGIN
  IF to_regclass('public.clinical_document_templates') IS NULL
    ...
    RAISE EXCEPTION 'clinical_referral_renderer_v1_foundation_missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'clinical_referral_renderer_v1_version_immutability_missing';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.clinical_document_template_versions v
    WHERE v.id = '12100000-0000-4000-8000-000000000007'::uuid
      AND v.template_id = '12000000-0000-4000-8000-000000000007'::uuid
      AND v.version = 1
      AND v.published_at IS NOT NULL
      AND v.definition = '{"kind":"referral","fields":["recipient","reason","clinical_summary","requested_action","priority","observations"]}'::jsonb
      AND v.render_definition = '{"layout":"clinical-document/plain-text-v1"}'::jsonb
  ) THEN
    RAISE EXCEPTION 'clinical_referral_renderer_v1_legacy_version_drift';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.clinical_document_template_versions v
    WHERE v.id = '12100000-0000-4000-8000-000000000016'::uuid
      AND v.template_id = '12000000-0000-4000-8000-000000000007'::uuid
      AND v.version = 2
      AND v.published_at IS NOT NULL
      AND v.definition = '{"kind":"referral","fields":["recipient","reason","clinical_summary","requested_action","priority","observations"]}'::jsonb
      AND (SELECT count(*) FROM jsonb_object_keys(v.render_definition)) = 6
      AND v.render_definition ?& ARRAY['layout','title','show_clinic_address','show_clinic_phone','show_patient_birth_date','show_specialty']
      AND v.render_definition->>'layout' = 'clinical-document/referral-v1'
      AND v.render_definition->>'title' = 'Encaminhamento clínico'
    ...
    RAISE EXCEPTION 'clinical_referral_renderer_v1_version_invalid';
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM public.clinical_document_templates t
    JOIN public.clinical_document_template_versions v ON v.id = t.current_version_id AND v.template_id = t.id
    WHERE t.id = '12000000-0000-4000-8000-000000000007'::uuid
      AND t.owner_type = 'platform'
      AND t.document_type = 'referral'
      AND t.status = 'active'
      AND v.id = '12100000-0000-4000-8000-000000000016'::uuid
      AND v.version = 2
      AND v.render_definition->>'layout' = 'clinical-document/referral-v1'
  ) THEN
    RAISE EXCEPTION 'clinical_referral_renderer_v1_current_version_missing';
  END IF;

  SELECT pg_get_functiondef('public.issue_clinical_document(uuid)'::regprocedure) INTO v_issue_def;
  IF v_issue_def NOT LIKE '%assert_clinical_document_actor%'
    ...
    RAISE EXCEPTION 'clinical_referral_renderer_v1_issue_snapshot_drift';
  END IF;
  ...
  IF has_table_privilege('authenticated', 'public.clinical_document_template_versions', 'INSERT')
    ...
    RAISE EXCEPTION 'clinical_referral_renderer_v1_direct_version_mutation_exposed';
  END IF;
END $$;

SELECT 'CLINICAL REFERRAL RENDERER V1 VERIFY PASSED' AS result;
ROLLBACK;
