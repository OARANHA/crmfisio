-- MedicsPro — D2-D2 production-safe verifier
...
BEGIN
  IF to_regclass('public.clinical_document_templates') IS NULL
    ...
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_foundation_missing';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_version_immutability_missing';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.clinical_document_template_versions v
    WHERE v.id = '12100000-0000-4000-8000-000000000006'::uuid
      AND v.template_id = '12000000-0000-4000-8000-000000000006'::uuid
      AND v.version = 1
      AND v.published_at IS NOT NULL
      AND v.definition = '{"kind":"exam_order","fields":["items","clinical_indication","impression","priority","observations"]}'::jsonb
      AND v.render_definition = '{"layout":"clinical-document/plain-text-v1"}'::jsonb
  ) THEN
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_legacy_version_drift';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM public.clinical_document_template_versions v
    WHERE v.id = '12100000-0000-4000-8000-000000000015'::uuid
      AND v.template_id = '12000000-0000-4000-8000-000000000006'::uuid
      AND v.version = 2
      AND v.published_at IS NOT NULL
      AND v.definition = '{"kind":"exam_order","fields":["items","clinical_indication","impression","priority","observations"]}'::jsonb
      ...
      AND v.render_definition->>'title' = 'Pedido de exames'
    ...
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_version_invalid';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_current_version_missing';
  END IF;
  ...
  IF v_issue_def NOT LIKE '%assert_clinical_document_actor%'
    ...
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_issue_snapshot_drift';
  END IF;
  ...
  IF has_table_privilege('authenticated', 'public.clinical_document_template_versions', 'INSERT')
    ...
    RAISE EXCEPTION 'clinical_exam_order_renderer_v1_direct_version_mutation_exposed';
  END IF;
END $$;

SELECT 'CLINICAL EXAM ORDER RENDERER V1 VERIFY PASSED' AS result;
ROLLBACK;
