-- MedicsPro — PHQ-15 Clinician-Assisted V1 installed-contract verifier.
BEGIN;
SET TRANSACTION READ ONLY;
SET LOCAL statement_timeout = '30s';

\echo 'VERIFY PHQ-15 clinician-assisted V1'

DO $$
BEGIN
  IF current_setting('transaction_read_only') <> 'on' THEN
    RAISE EXCEPTION 'phq15_v1_verifier_not_read_only';
  END IF;
  IF current_setting('server_version_num')::integer < 160000 THEN
    RAISE EXCEPTION 'phq15_v1_postgresql_16_required';
  END IF;
  IF to_regclass('public.nexus_result_contracts') IS NULL
    ...
    RAISE EXCEPTION 'phq15_v1_prerequisite_missing';
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE module_key='scales' AND tool_key='phq15'
      AND rule_key='nexus.phq15'
      AND rule_version='nexus-phq15-2026-09-13'
      AND required_capability='nexus.scales'
  ) THEN
    RAISE EXCEPTION 'phq15_v1_nexus_contract_missing';
  END IF;
  IF public.resolve_nexus_result_required_capability(
       'scales','phq15','nexus.phq15','nexus-phq15-2026-09-13'
     ) IS DISTINCT FROM 'nexus.scales' THEN
    RAISE EXCEPTION 'phq15_v1_resolver_drift';
  END IF;
...
BEGIN
  ...
  IF v_default IS NULL OR lower(v_default) NOT IN ('false', 'false::boolean') THEN
    RAISE EXCEPTION 'phq15_v1_setting_default_not_deny';
  END IF;

  SELECT pg_get_functiondef('public.set_clinic_clinical_instrument_enabled(text,boolean)'::regprocedure)
    INTO v_src;
  IF position('current_clinic_id()' in v_src)=0
     ...
     OR position('clinical_instrument_catalog' in v_src)=0 THEN
    RAISE EXCEPTION 'phq15_v1_setting_rpc_drift';
  END IF;
  ...
  IF position('clinic_clinical_instrument_settings' in v_src)=0
    ...
    RAISE EXCEPTION 'phq15_v1_neutral_authorization_drift';
  END IF;
  ...
  IF position('can_apply_clinical_instrument_in_encounter' in v_src)=0
    ...
    RAISE EXCEPTION 'phq15_v1_writer_authorization_drift';
  END IF;
END;
$$;
...
BEGIN
  IF has_table_privilege('authenticated','public.nexus_result_contracts','SELECT')
     OR has_table_privilege('authenticated','public.clinical_instrument_catalog','SELECT')
     OR has_table_privilege('anon','public.clinical_instrument_catalog','SELECT') THEN
    RAISE EXCEPTION 'phq15_v1_catalog_acl_drift';
  END IF;
END;
$$;

\echo 'PHQ15 CLINICIAN ASSISTED V1 VERIFY PASSED'
ROLLBACK;
