...
\echo 'VERIFY #399 — neutral instrument catalog + clinical base + Apply in Encounter authorization'
...
-- 1) Capability and neutral exposure catalog shape.
DO $$
DECLARE
  v_default text;
BEGIN
  ...
  IF to_regclass('public.clinical_instrument_catalog') IS NULL THEN
    RAISE EXCEPTION 'ci399_neutral_catalog_missing';
  END IF;
  ...
  IF (
    ...
    WHERE table_schema='public'
      AND table_name='clinical_instrument_catalog'
      AND (
        (column_name='instrument_key' AND data_type='text' AND is_nullable='NO')
        OR (column_name='engine_source' AND data_type='text' AND is_nullable='NO')
        OR (column_name='engine_module_key' AND data_type='text' AND is_nullable='NO')
        OR (column_name='engine_tool_key' AND data_type='text' AND is_nullable='NO')
        OR (column_name='engine_rule_key' AND data_type='text' AND is_nullable='NO')
        OR (column_name='engine_rule_version' AND data_type='text' AND is_nullable='NO')
        OR (column_name='active' AND data_type='boolean' AND is_nullable='NO')
        OR (column_name='created_at' AND data_type='timestamp with time zone' AND is_nullable='NO')
      )
  ) <> 8 THEN
    RAISE EXCEPTION 'ci399_neutral_catalog_column_shape_drift';
  END IF;
  ...
  IF lower(coalesce(v_default,'')) NOT IN ('false','false::boolean') THEN
    RAISE EXCEPTION 'ci399_neutral_catalog_active_default_must_be_false: %',v_default;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint c
    WHERE c.conrelid='public.clinical_instrument_catalog'::regclass
      AND c.contype='f'
      AND c.conname='clinical_instrument_catalog_engine_contract_fkey'
  ) THEN
  ...
  IF (SELECT count(*) FROM public.clinical_instrument_catalog WHERE active IS TRUE) <> 2
     OR NOT EXISTS (
       ...
       WHERE instrument_key='phq9'
         ...
         AND engine_tool_key='phq9'
         AND engine_rule_key='nexus.phq9'
         AND engine_rule_version='nexus-2026-09-03'
         AND active IS TRUE
     )
     OR NOT EXISTS (
       SELECT 1 FROM public.clinical_instrument_catalog
       WHERE instrument_key='gad7'
         AND engine_source='nexus'
         AND engine_module_key='scales'
         AND engine_tool_key='gad7'
         AND engine_rule_key='nexus.gad7'
         AND engine_rule_version='nexus-2026-09-03'
         AND active IS TRUE
     ) THEN
    RAISE EXCEPTION 'ci399_neutral_catalog_expected_exposure_drift';
  END IF;

  IF has_table_privilege('authenticated','public.clinical_instrument_catalog','SELECT')
     OR has_table_privilege('authenticated','public.clinical_instrument_catalog','INSERT')
     OR has_table_privilege('authenticated','public.clinical_instrument_catalog','UPDATE')
     OR has_table_privilege('authenticated','public.clinical_instrument_catalog','DELETE')
     OR has_table_privilege('anon','public.clinical_instrument_catalog','SELECT') THEN
    RAISE EXCEPTION 'ci399_neutral_catalog_browser_acl_drift';
  END IF;
END;
$$;

-- 2) Tenant configuration shape and binding to neutral exposure catalog.
DO $$
DECLARE
  v_default text;
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE c.conrelid='public.clinic_clinical_instrument_settings'::regclass
      AND c.contype='f'
      AND c.conname='clinic_clinical_instrument_settings_catalog_fkey'
  ) THEN
    RAISE EXCEPTION 'ci399_settings_key_contract_missing';
  END IF;
END;
$$;

-- 3) Configuration RLS/ACL: browser reads tenant-scoped settings but writes only via RPC.
DO $$
DECLARE
  v_qual text;
BEGIN
...
-- 4) Setting validation and the only browser configuration writer must consult
-- the neutral catalog, never Nexus registry membership as exposure authority.
DO $$
DECLARE
  p record;
  v_src text;
BEGIN
  ...
  IF position('clinical_instrument_catalog' in v_src)=0
     OR position('c.active IS TRUE' in v_src)=0
     OR position('nexus_result_contracts' in v_src)>0 THEN
    RAISE EXCEPTION 'ci399_setting_validator_exposure_drift';
  END IF;
...
-- 5) Base authorization is internal and neutral; it cannot be mistaken for a
...
-- 6) Contextual boundary is specifically Apply in active Encounter.
DO $$
DECLARE
  p record;
  v_src text;
...
-- 7) Engine provenance remains Nexus, but exposure remains the neutral allowlist.
-- No questions, validation, scoring or result persistence are duplicated here.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.clinical_instrument_catalog c
    LEFT JOIN public.nexus_result_contracts n
      ON n.module_key=c.engine_module_key
     AND n.tool_key=c.engine_tool_key
     AND n.rule_key=c.engine_rule_key
     AND n.rule_version=c.engine_rule_version
    WHERE c.engine_source='nexus'
      AND n.module_key IS NULL
  ) THEN
    RAISE EXCEPTION 'ci399_neutral_catalog_engine_link_broken';
  END IF;
...
\echo 'VERIFY #399 OK — neutral exposure catalog + tenant enablement + Apply in Encounter boundary'
ROLLBACK;
