...
- PCL-5 — PROD / VERIFIED;
- PC-PTSD-5 — #495 PROD / VERIFIED / tenant enablement required; tradução operacional PT-BR explicitamente não apresentada como validação brasileira.
...
- **PC-PTSD-5:** #495 is PROD / VERIFIED and still requires explicit tenant enablement. It preserves the trauma gate + five binary symptom items, uses an operational PT-BR translation with no Brazilian validation claim, freezes cutoff `>=4` from external evidence, creates no `patient_self` contract and does not auto-enable any tenant.
