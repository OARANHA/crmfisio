...
- PCL-5 — PROD / VERIFIED;
- PC-PTSD-5 — #495 mergeada / main validada / não PROD; tradução operacional PT-BR explicitamente não apresentada como validação brasileira.
...
- **PC-PTSD-5:** #495 is merged/main-validated and still not PROD. It preserves the trauma gate + five binary symptom items, uses an operational PT-BR translation with no Brazilian validation claim, freezes cutoff `>=4` from external evidence, creates no `patient_self` contract and does not auto-enable any tenant.
