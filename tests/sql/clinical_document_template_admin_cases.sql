...
-- Admin A has no clinical identity, but may administer its own tenant templates.
BEGIN;
...
ORDER BY template_id LIMIT 1 \gset
...
('admin_without_crm_can_create', EXISTS (
  SELECT 1 FROM public.list_clinical_document_templates_for_management('medication_prescription')
  WHERE template_id = :'d2b2a_clinic_template'::uuid
    AND owner_type = 'clinic' AND read_only = false AND current_version = 1
)),
('platform_clone_becomes_independent_clinic_template', EXISTS (
  ...
  WHERE template_id = :'d2b2a_clone_template'::uuid
    ...
    AND current_version = 1 AND read_only = false
));

SELECT pg_temp.d2b2a_expect_error(
  'admin_cannot_issue_by_management_role',
  format('SELECT public.create_clinical_document_draft(%L::uuid,%L::uuid,%L::jsonb)',
...
SELECT pg_temp.d2b2a_expect_error(
  ...
  format('INSERT INTO public.clinical_document_template_versions(template_id,version,definition,render_definition,variables_contract) VALUES(%L::uuid,99,%L::jsonb,%L::jsonb,%L::jsonb)',
...
SELECT pg_temp.d2b2a_expect_error(
  ...
  format('SELECT public.update_clinic_clinical_document_template_metadata(%L::uuid,%L,%L,%L::jsonb,%L)',
...
SELECT pg_temp.d2b2a_expect_error(
  ...
  format('SELECT public.publish_clinic_clinical_document_template_version(%L::uuid,%L::jsonb,%L::jsonb,%L::jsonb)',
         :'d2b2a_clinic_template',
         '{"kind":"medication_prescription","fields":["items"]}',
...
SELECT pg_temp.d2b2a_expect_error(
  ...
  format('SELECT public.publish_clinic_clinical_document_template_version(%L::uuid,%L::jsonb,%L::jsonb,%L::jsonb)',
         ...
         '{"layout":"clinical-document/plain-text-v1"}', '["patient.name"]'),
  'clinical_document_template_fields_invalid'
);
COMMIT;

-- Physician can issue from the clinic-owned version but cannot administer it.
BEGIN;
SET LOCAL ROLE authenticated;
SET LOCAL row_security = on;
SELECT set_config('request.jwt.claim.role','authenticated',true);
...
SELECT (public.create_clinical_document_draft(
  'd2300000-0000-4000-8000-000000000001', :'d2b2a_v1'::uuid,
...
-- Admin publishes v2. Historical prescription must remain pinned to v1 snapshot.
BEGIN;
SET LOCAL ROLE authenticated;
SET LOCAL row_security = on;
SELECT set_config('request.jwt.claim.role','authenticated',true);
...
SELECT pg_temp.d2b2a_expect_error(
  ...
  format('UPDATE public.clinical_document_template_versions SET definition=%L::jsonb WHERE id=%L::uuid',
...
-- Owner may manage independently of clinical identity.
BEGIN;
SET LOCAL ROLE authenticated;
SET LOCAL row_security = on;
SELECT set_config('request.jwt.claim.role','authenticated',true);
...
-- Non-manager roles and an unscoped/platform-only actor stay denied.
BEGIN;
SET LOCAL ROLE authenticated;
SET LOCAL row_security = on;
SELECT set_config('request.jwt.claim.role','authenticated',true);
...
-- Archive is administrative visibility only; no historical deletion.
BEGIN;
SET LOCAL ROLE authenticated;
SET LOCAL row_security = on;
SELECT set_config('request.jwt.claim.role','authenticated',true);
...
SELECT public.update_clinic_clinical_document_template_metadata(
  :'d2b2a_clinic_template'::uuid,
  'Receita da clínica atualizada', 'Arquivada administrativamente',
  '{"specialty":"clinica_medica"}'::jsonb, 'archived'
);
COMMIT;
...
BEGIN
  ...
  IF v_failed IS NOT NULL THEN
    RAISE EXCEPTION 'clinical_document_template_admin_cases_failed: %', v_failed;
  END IF;
END $$;
...
SELECT 'CLINICAL DOCUMENT TEMPLATE ADMIN BEHAVIOR PASSED' AS result;
